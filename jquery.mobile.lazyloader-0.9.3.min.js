(function(b){b.widget("mobile.lazyloader",b.mobile.widget,{$element:null,$page:null,$ul:null,_defaultOptions:{threshold:360,retrieve:20,retrieved:20,bubbles:!1,offset:0,transform:"",icanhaz:""},_defaultParameters:{retrieve:20,retrieved:20,offset:0},_defaultSettings:{pageId:"",ulId:"",progressDivId:"",moreUrl:"",clearUrl:""},_handleScrollStartJustFired:!1,_handleScrollStopJustFired:!1,_mouseWheelEventJustFired:!1,_handleScrollStartTimeoutId:null,_handleScrollStopTimeoutId:null,_mouseWheelTimeoutId:null,
_instances:{},parameters:null,_settings:null,timeoutOptions:{mousewheel:400,scrollstart:600,scrollstop:60,showprogress:200,scrolldown:400,immediately:0},_widgetName:"lazyloader",_widgetState:{busy:!1,done:!1},_create:function(){this._initialize(this._defaultOptions,this._defaultSettings,this._defaultParameters);this._bind()},_init:function(){},_initialize:function(a,d,c){"undefined"!=typeof a&&""!=a&&(this._widgetState.busy=!1,this._widgetState.done=!1,this._settings=b.extend(!0,this._settings,this._defaultSettings),
...
a+("&"+e+"="+$that.parameters[e]),c+=1;b.ajax({type:"POST",url:$that._settings.moreUrl,async:!0,data:a,success:function(a){var c=b.parseJSON(a),a=c.data[0].count,d="",e="",h="",j="",g="",k="",f="";if(a>0){g="#"+$that._settings.ulId;k="#"+$that._settings.ulId+" li";f=$that._getBottomElement(g,'[data-role="list-divider"]');if(typeof c.data[0].html!="undefined"&&c.data[0].html!=""){d=c.data[0].html;f?b(k).last().before(d):b(g).append(d)}else{e=c.data[0].json;if(typeof e!="undefined"&&e!=""){if(typeof $that.options.transform!=
"undefined"&&$that.options.transform!="")h=$that.options.transform;if($that._instances[$that._settings.pageId]&&typeof $that._instances[$that._settings.pageId].transform!="undefined"&&$that._instances[$that._settings.pageId].transform!="")h=$that._instances[$that._settings.pageId].transform;if(typeof $that.options.icanhaz!="undefined"&&$that.options.icanhaz!="")j=$that.options.icanhaz;if($that._instances[$that._settings.pageId]&&typeof $that._instances[$that._settings.pageId].icanhaz!="undefined"&&
$that._instances[$that._settings.pageId].icanhaz!="")j=$that._instances[$that._settings.pageId].icanhaz;if(j!=""){ich.addTemplate("listitem",j);for(i=0;i<e.length;i++){c=ich.listitem(e[i],true);d=d+c}ich.clearAll();f?b(k).last().before(d):b(g).append(d)}else if(h!=""){f&&f.remove();b(g).json2html(e,h);f&&b(k).last().append(f)}}}b(g).listview("refresh");$that._instances[$that._settings.pageId].retrieved=$that._instances[$that._settings.pageId].retrieved+parseInt(a);if(a<$that.options.retrieve||$that.options.retrieve==
"all"){$that._widgetState.done=true;$that._trigger("alldone",{type:"lazyloaderalldone","function":"_load",pageId:$that._settings.pageId,ulId:$that._settings.ulId,loaded:$that.options.retrieved})}}else{$that._widgetState.done=true;$that._trigger("alldone",{type:"lazyloaderalldone","function":"_load",pageId:$that._settings.pageId,ulId:$that._settings.ulId,loaded:$that.options.retrieved})}b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=false});$that._trigger("doneloading",
{type:"lazyloaderdoneloading","function":"_load",pageId:$that._settings.pageId,ulId:$that._settings.ulId,loaded:$that.options.retrieved})},error:function(a){$that._trigger("error",{type:"lazyloadererror","function":"_load",message:a,settings:$that._settings,options:$that.options,parameters:$that.parameters});b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=false})}})})},a))},_getBottomElement:function(a,d){var c=b(a).find(d);switch(c.length){case 2:c=c.last();break;
default:c=null}return"undefined"!=typeof c&&null!=c&&""!=c&&"null"!=c?c:!1},_handleMouseWheelEvent:function(){if(!this._mouseWheelEventJustFired&&!this._handleScrollStopJustFired&&!this._handleScrollStartJustFired){this._mouseWheelEventJustFired=!0;this._load(this.timeoutOptions.mousewheel);var a=this;this._mouseWheelTimeoutId=setTimeout(function(){a._mouseWheelEventJustFired=!1},1E3)}},_handleScrollStart:function(){if(!this._mouseWheelEventJustFired&&!this._handleScrollStopJustFired&&!this._handleScrollStartJustFired){this._handleScrollStartJustFired=
...
this._load(this.timeoutOptions.scrolldown)},_setOption:function(a,d){this._instances[this._settings.pageId]&&this._instances[this._settings.pageId][a]&&(this._instances[this._settings.pageId][a]=d);b.Widget.prototype._setOption.apply(this,arguments)},refresh:function(a,d){if("parameters"==a){if("undefined"!=typeof this.options)for(var c in this.parameters)"undefined"!=typeof this.options[c]&&(this.parameters[c]=this.options[c])}else"parameter"==a&&(c=d,"undefined"!=typeof this.options[c]&&(this.parameters[c]=
this.options[c]));c=JSON.stringify(this.parameters);this.parameters=b.parseJSON(c)},reInitialize:function(a,b,c){this._initialize(a,b,c)},reset:function(a){var d=this;b.ajax({type:"POST",url:d._settings.clearUrl,async:!0,data:"section="+a,success:function(b){parseInt(b)&&(d.options.retrieved=d._defaultOptions.retrieved,d._widgetState.done=!1,"undefined"!=typeof d._instances[a]&&delete d._instances[a],d._trigger("reset",{type:"lazyloaderreset","function":"reset",pageId:a,settings:d._settings,options:d.options,
parameters:d.parameters}))},error:function(a){d._trigger("error",{type:"lazyloadererror","function":"reset",message:a,settings:d._settings,options:d.options,parameters:d.parameters});b("#"+d._settings.progressDivId).hide(250,function(){d._widgetState.busy=!1})}})},resetAll:function(){var a=this;b.ajax({type:"POST",url:a._settings.clearUrl,async:!0,data:"",success:function(b){if(parseInt(b)){for(pageId in a._instances)a._instances[pageId]&&delete a._instances[pageId];a.options.retrieved=a._defaultOptions.retrieved;
a._widgetState.done=!1;a._widgetState.busy=!1;a._trigger("resetall",{type:"lazyloaderresetall","function":"resetAll",_widgetState:a._widgetState,_instances:a._instances})}}})}});b(document).bind("pagecreate create",function(a){b.mobile.lazyloader.prototype.enhanceWithin(a.target)})})(jQuery);
