(function(b,n){b.widget("mobile.lazyloader",b.mobile.widget,{_defaultOptions:{threshold:360,retrieve:20,retrieved:20,bubbles:!1,offset:0},_defaultParameters:{retrieve:20,retrieved:20,offset:0},_defaultSettings:{pageId:"",templateType:"",templatePrecompiled:!1,templateId:"",template:"",mainId:"",progressDivId:"",moreUrl:"",clearUrl:""},_defaultSelectors:{main:"ul",single:"ul li",bottom:'[data-role="list-divider"]'},_handleScrollStartJustFired:!1,_handleScrollStopJustFired:!1,_mouseWheelEventJustFired:!1,
...
$that._instances[$that._settings.pageId].options.offset):($that._parameters.retrieve=$that.options.retrieve,$that._parameters.retrieved=$that.options.retrieved,$that._parameters.offset=$that.options.offset);if("undefined"!=typeof $that._settings.pageId&&""!=$that._settings.pageId){var e=b("#"+$that._settings.pageId).find('[type="hidden"]');for(i=0;i<e.length;i++){var g=b(e).get(i);"undefined"!=typeof b(g).attr("id")&&""!=b(g).attr("id")&&($that._parameters[b(g).attr("id")]=escape(b(g).val()))}}for(var h in $that._parameters)a=
0==c?a+(h+"="+$that._parameters[h]):a+("&"+h+"="+$that._parameters[h]),c+=1;b.ajax({type:"POST",url:moreUrl,async:!0,data:a,success:function(a){var a=b.parseJSON(a),c=a.data[0].count,d="",e="",f="",k="",m="",g=false,l="",h="",j=e="";if(c>0){l=$that._selectors.main;h=$that._selectors.single;e=$that._selectors.bottom;j=$that._getBottomElement(l,e);if(typeof a.data[0].html!="undefined"&&a.data[0].html!=""){d=a.data[0].html;j?b(h).last().before(d):b(l).append(d)}else{if($that._instances[$that._settings.pageId]){if(typeof $that._instances[$that._settings.pageId].settings.templateId!=
"undefined"&&$that._instances[$that._settings.pageId].settings.templateId!=""){k=$that._instances[$that._settings.pageId].settings.templateId;if(typeof $that._instances[$that._settings.pageId].settings.templateType!="undefined"&&$that._instances[$that._settings.pageId].settings.templateType!="")m=$that._instances[$that._settings.pageId].settings.templateType;if(typeof $that._instances[$that._settings.pageId].settings.template!="undefined"&&$that._instances[$that._settings.pageId].settings.template!=
"")f=$that._instances[$that._settings.pageId].settings.template}g=$that._instances[$that._settings.pageId].settings.templatePrecompiled}else{if(typeof $that._settings.templateId!="undefined"&&$that._settings.templateId!=""){k=$that._settings.templateId;if(typeof $that._settings.templateType!="undefined"&&$that._settings.templateType!="")m=$that._settings.templateType;if(typeof $that._settings.template!="undefined"&&$that._settings.template!="")f=$that._settings.template}g=$that._settings.templatePrecompiled}if(m!==
""&&k!==""&&f!=="")if(m==="json2html"){e=a.data[0].json;j&&j.remove();b(l).json2html(e,b.parseJSON(f));j&&b(h).last().append(j)}else{e=a.data[0];switch(m){case "handlebars":f=g?Handlebars.templates[k+".tmpl"]:Handlebars.compile(f);d=f(e);break;case "icanhaz":ich.addTemplate("listitem",f);d=ich.listitem(e,true);ich.clearAll();break;case "dust":if(g)dust.render(k,e,function(a,b){d=b});else{dust.loadSource(f);dust.render(k,e,function(a,b){d=b})}break;case "dot":f=doT.template(f);d=f(e)}j?b(h).last().before(d):
b(l).append(d)}}b(l).listview("refresh");$that._instances[$that._settings.pageId].options.retrieved=$that._instances[$that._settings.pageId].options.retrieved+parseInt(c);if(c<$that.options.retrieve||$that.options.retrieve=="all"){$that._widgetState.done=true;$that._triggerEvent("alldone","_load")}}else{$that._widgetState.done=true;$that._triggerEvent("alldone","_load")}b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=false});$that._triggerEvent("doneloading","_load")},
error:function(a){$that._triggerEvent("error","_load",a);b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=false})}})})},a)):this._widgetState.done?$that._triggerEvent("alldone","_load"):this._widgetState.busy&&$that._triggerEvent("busy","_load"):b("#"+this._settings.progressDivId).hide(250,function(){"undefined"!=typeof this._widgetState&&(this._widgetState.busy=!1)}))},_getBottomElement:function(a,d){var c=b(a).last().find(d);switch(c.length){case 2:c=c.last();break;
