...
if("undefined"!=typeof $that._settings.pageId&&""!=$that._settings.pageId){var a=b("#"+$that._settings.pageId).find('[type="hidden"]');for(i=0;i<a.length;i++){var d=b(a).get(i);"undefined"!=typeof b(d).attr("id")&&""!=b(d).attr("id")&&($that.parameters[b(d).attr("id")]=escape(b(d).val()))}}var a="",e=0,f;for(f in $that.parameters)a=0==e?a+(f+"="+$that.parameters[f]):a+("&"+f+"="+$that.parameters[f]),e+=1;b.ajax({type:"POST",url:$that._settings.moreUrl,async:!0,data:a,success:function(a){more=b.parseJSON(a);
e=more.data[0].count;html=more.data[0].html;if(e>0){b("#"+$that._settings.ulId+" li").last().before(html);b("#"+$that._settings.ulId).listview("refresh");$that._instances[$that._settings.pageId].retrieved=$that._instances[$that._settings.pageId].retrieved+parseInt(e);if(e<$that.options.retrieve){$that._widgetState.done=true;$that._trigger("doneloading",{type:"lazyloaderdone","function":"_load",pageId:$that._settings.pageId,ulId:$that._settings.ulId,loaded:$that.options.retrieved})}}else{$that._widgetState.done=
true;$that._trigger("doneloading",{type:"lazyloaderdone","function":"_load",pageId:$that._settings.pageId,ulId:$that._settings.ulId,loaded:$that.options.retrieved})}b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=false})},error:function(a){$that._trigger("error",{type:"lazyloadererror","function":"_load",message:a,settings:$that._settings,options:$that.options,parameters:$that.parameters});b("#"+$that._settings.progressDivId).hide(250,function(){$that._widgetState.busy=
false})}})})},a))},_handleMouseWheelEvent:function(){if(!this._mouseWheelEventJustFired&&!this._handleScrollStopJustFired&&!this._handleScrollStartJustFired){this._mouseWheelEventJustFired=!0;this._load(this.timeoutOptions.mousewheel);var a=this;this._mouseWheelTimeoutId=setTimeout(function(){a._mouseWheelEventJustFired=!1},1E3)}},_handleScrollStart:function(){if(!this._mouseWheelEventJustFired&&!this._handleScrollStopJustFired&&!this._handleScrollStartJustFired){this._handleScrollStartJustFired=
...
this._load(this.timeoutOptions.scrolldown)},_setOption:function(a,c){this._instances[this._settings.pageId]&&this._instances[this._settings.pageId][a]&&(this._instances[this._settings.pageId][a]=c);b.Widget.prototype._setOption.apply(this,arguments)},refresh:function(a,c){if("parameters"==a){if("undefined"!=typeof this.options)for(var d in this.parameters)"undefined"!=typeof this.options[d]&&(this.parameters[d]=this.options[d])}else"parameter"==a&&(d=c,"undefined"!=typeof this.options[d]&&(this.parameters[d]=
this.options[d]));d=JSON.stringify(this.parameters);this.parameters=b.parseJSON(d)},reInitialize:function(a,b,d){this._initialize(a,b,d)},reset:function(a){var c=this;b.ajax({type:"POST",url:c._settings.clearUrl,async:!0,data:"section="+a,success:function(b){parseInt(b)&&(c.options.retrieved=c._defaultOptions.retrieved,c._widgetState.done=!1,"undefined"!=typeof c._instances[a]&&delete c._instances[a],c._trigger("reset",{type:"lazyloaderreset","function":"reset",pageId:a,settings:c._settings,options:c.options,
parameters:c.parameters}))},error:function(a){c._trigger("error",{type:"lazyloadererror","function":"reset",message:a,settings:c._settings,options:c.options,parameters:c.parameters});b("#"+c._settings.progressDivId).hide(250,function(){c._widgetState.busy=!1})}})},resetAll:function(){var a=this;b.ajax({type:"POST",url:a._settings.clearUrl,async:!0,data:"",success:function(b){if(parseInt(b)){for(pageId in a._instances)a._instances[pageId]&&delete a._instances[pageId];a.options.retrieved=a._defaultOptions.retrieved;
